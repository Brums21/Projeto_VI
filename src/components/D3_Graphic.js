...
function D3_Graphic({ data, station, meteorology, vehicle_type, date, graph_type}) {
  ...
  const fetchData = async () => {
    try {
      ...
      if (graph_type === "speed_meteorological"){
        ...
        const result2 = await queryInfluxDB(cpms("totalCount", "mean"));
        setQueryResult2(result2);

        const result3 = await queryInfluxDB(cpms("totalCount", "mean"));
        setQueryResult3(result2);

        const result4 = await queryInfluxDB(cpms("totalCount", "mean"));
        setQueryResult4(result2);

        const result5 = await queryInfluxDB(cpms("totalCount", "mean"));
        setQueryResult5(result2);

        const result6 = await queryInfluxDB(cpms("totalCount", "mean"));
        setQueryResult6(result2);
      }
    } catch (error) {
  ...
  //TODO
  function createThirdGraph(containerId, totalCount, avgSpeed, maxAvgSpeed, minAvgSpeed, maxSpeed, minSpeed) {
